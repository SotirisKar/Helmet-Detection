...
while(video.isOpened()):
    ...
    if 'motorcycle' in object_list: 
        if object_list.count('motorcycle') == object_list.count('helmet'):
            cv2.putText(frame,' Wears Helmet',(15,160),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2,cv2.LINE_AA)
        elif object_list.count('motorcycle') > object_list.count('helmet'):
            cv2.putText (frame, 'No Helmet',(15,160),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2,cv2.LINE_AA)
    '''
    x = None
    if 'motorcycle' in object_list: 
        if object_list.count('motorcycle') == object_list.count('helmet'):
