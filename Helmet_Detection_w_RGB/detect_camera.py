...
from utils.display import displayRGB
...
parser.add_argument('--model', help='Provide the path to the TFLite file, default is models/model.tflite',
                    default='/home/pi/helmet-detection/saved_models/model.tflite')
parser.add_argument('--labels', help='Provide the path to the Labels, default is models/labels.txt',
                    default='/home/pi/helmet-detection/saved_models/labels.txt')
...
while True:
    ...
    if 'motorcycle' in object_list: 
        if object_list.count('motorcycle') == object_list.count('helmet'):
            x = 'Wears Helmet'
        if object_list.count('helmet') >= object_list.count('motorcycle') and object_list.count('motorcycle') >= 1:
            x = 'Wears Helmet'
    ...
    # Adjust Delay:(+10 to +50)
    if x == 'No Helmet':
        delay_no = fps_count + 10
    elif x == 'Wears Helmet':
        delay_yes = fps_count + 10
    ...
    if x == 'Wears Helmet':
        ...
        image = '/home/pi/helmet-detection/thumbnails/helmet.jpg'
        dis = Process(target=displayRGB, args=[image])
        dis.daemon = True
        if not dis.is_alive():
            dis.start()
        else:
        ...
        image = '/home/pi/helmet-detection/thumbnails/no_helmet.jpg'
        dis = Process(target=displayRGB, args=[image])
        dis.daemon = True
        if not dis.is_alive():
            dis.start()
